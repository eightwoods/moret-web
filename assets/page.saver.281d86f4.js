import{k as f,u as $,v,p as D,t as N,d as k,s as I,a as E,x as A,y as q}from"./main.cb79c6a8.js";import F from"./component.chartComparison.2145fb22.js";import P from"./component.percentageBarMulti.531ea37c.js";var H={globals:{elem:document.querySelector(".saver")},init(){document.querySelector(".saver-list-content .js-refresh").addEventListener("click",()=>this.setSavers()),document.querySelector("body.mobile.unknown")&&this.setSavers();const e={childList:!0,attributes:!0,attributeFilter:["sidenav-activechange","sidenav-refreshprice"]};new MutationObserver(t=>{console.log("sidenav refreshed from Saver!");for(let n of t)if(n.type==="attributes")switch(n.attributeName){case"sidenav-activechange":document.querySelector("body.mobile.unknown")||this.setSavers();break}this.globals.init=!1}).observe(this.globals.elem.querySelector(".sidenav"),e)},setSavers(){console.log("setSavers()");const e=document.querySelector(".saver-list"),a=document.querySelector(".saver-info");f(e),f(a);const t=e.querySelector(".comp-dynamic-table");t.innerHTML=`
            ...
            </div>`,this.setSaverInfo(),$().then(n=>{const i=[],u=[];let m=0;n.forEach(async(s,h)=>{try{const o=await v(s,"name"),l=await v(s,"symbol"),c=await v(s,"aum"),b=await v(s,"supply"),g=await v(s,"balance"),r=await v(s,"params"),d=await v(s,"vintage"),p=await v(s,"opentime"),w=b>0?c/b:1,T=`$${(b>0?c/b*g:0).toFixed(2)}`,y=d.StartLevel>0?c/d.StartLevel-1:0,S=new Date((p+Number(r.tradeWindow))*1e3).toLocaleString(),x=p<Math.floor(Date.now()/1e3);u.push({Name:o,Symbol:l,Address:s,Description:`Soft Ceiling at ${(Number(r.callMoney)/Number(r.multiplier)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0})} rolled every ${Number(r.callTenor)/86400} days <br>Buffer range ${(Number(r.putSpread)/Number(r.multiplier)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0})} to ${(Number(r.putMoney)/Number(r.multiplier)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0})} rolled every ${Number(r.putTenor)/86400} days`,MarketCap:`$${c.toFixed(2)}`,UnitAsset:w,Holding:T,Yield:((Number(r.callMoney)/Number(r.multiplier)-1)*365/(Number(r.putTenor)/86400)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0}),ProfitLoss:y,NextVintageStart:S,ThisVintageEnd:new Date(p*1e3).toLocaleString(),VintageOpen:x,StartLevel:d.StartLevel,Upside:d.Upside,Downside:d.Downside,Protection:d.Protection,Tenor:r.callTenor}),i.push([o,T,w,y.toLocaleString(void 0,{style:"percent",minimumFractionDigits:0}),x?"Open":"Closed",S]),m++,m===n.length&&(f(e,!1),f(a,!1),this.setSaverInfo(u[0]),D.setDynamic(t,i),n.length>1&&t.querySelectorAll("tbody tr").forEach((L,C)=>{L.classList.add("cursor"),L.addEventListener("click",()=>{this.setSaverInfo(u[C])},!1)}))}catch(o){console.error(o),console.log("Fail! refresh data load...");const l=setTimeout(()=>{this.setSavers(),clearTimeout(l)},5e3)}})}).catch(n=>{console.error(n)})},setSaverInfo(e){const a=document.querySelector(".saver-info-content");if(!e){a.textContent="";return}a.innerHTML=`
            <div class="saver-row">
                <div class="saver-col">
