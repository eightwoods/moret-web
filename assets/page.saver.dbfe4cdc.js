import{k as f,u as $,v,p as D,t as N,d as I,s as E,a as k,x as A,y as F}from"./main.91084b8b.js";import q from"./component.chartComparison.2145fb22.js";import P from"./component.percentageBarMulti.5a470680.js";var H={globals:{elem:document.querySelector(".saver")},init(){document.querySelector(".saver-list-content .js-refresh").addEventListener("click",()=>this.setSavers());const e={childList:!0,attributes:!0,attributeFilter:["sidenav-activechange","sidenav-refreshprice"]};new MutationObserver(t=>{console.log("sidenav refreshed from Saver!");for(let n of t)if(n.type==="attributes")switch(n.attributeName){case"sidenav-activechange":this.setSavers();break}this.globals.init=!1}).observe(this.globals.elem.querySelector(".sidenav"),e)},setSavers(){console.log("setSavers()");const e=document.querySelector(".saver-list"),a=document.querySelector(".saver-info");f(e),f(a);const t=e.querySelector(".comp-dynamic-table");t.innerHTML=`
            ...
            </div>`,this.setSaverInfo(),$().then(n=>{const i=[],u=[];let p=0;n.forEach(async(r,h)=>{try{const o=await v(r,"name"),l=await v(r,"symbol"),c=await v(r,"aum"),g=await v(r,"supply"),b=await v(r,"balance"),s=await v(r,"params"),d=await v(r,"vintage"),m=await v(r,"opentime"),w=g>0?c/g:1,T=`$${(g>0?c/g*b:0).toFixed(2)}`,S=d.StartLevel>0?c/d.StartLevel-1:0,y=new Date((m+Number(s.tradeWindow))*1e3).toLocaleString(),x=m<Math.floor(Date.now()/1e3);u.push({Name:o,Symbol:l,Address:r,Description:`Soft Ceiling at ${(Number(s.callMoney)/Number(s.multiplier)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0})} rolled every ${Number(s.callTenor)/86400} days <br>Buffer range ${(Number(s.putSpread)/Number(s.multiplier)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0})} to ${(Number(s.putMoney)/Number(s.multiplier)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0})} rolled every ${Number(s.putTenor)/86400} days`,MarketCap:`$${c.toFixed(2)}`,UnitAsset:w,Holding:T,Yield:((Number(s.callMoney)/Number(s.multiplier)-1)*365/(Number(s.putTenor)/86400)).toLocaleString(void 0,{style:"percent",minimumFractionDigits:0}),ProfitLoss:S,NextVintageStart:y,ThisVintageEnd:new Date(m*1e3).toLocaleString(),VintageOpen:x,StartLevel:d.StartLevel,Upside:d.Upside,Downside:d.Downside,Protection:d.Protection,Tenor:s.callTenor}),i.push([o,T,w,S.toLocaleString(void 0,{style:"percent",minimumFractionDigits:0}),x?"Open":"Closed",y]),p++,p===n.length&&(f(e,!1),f(a,!1),this.setSaverInfo(u[0]),D.setDynamic(t,i),n.length>1&&t.querySelectorAll("tbody tr").forEach((L,C)=>{L.classList.add("cursor"),L.addEventListener("click",()=>{this.setSaverInfo(u[C])},!1)}))}catch(o){console.error(o),console.log("Fail! refresh data load...");const l=setTimeout(()=>{this.setSavers(),clearTimeout(l)},5e3)}})}).catch(n=>{console.error(n)})},setSaverInfo(e){const a=document.querySelector(".saver-info-content");if(!e){a.textContent="";return}a.innerHTML=`
            <div class="saver-row">
                <div class="saver-col">
