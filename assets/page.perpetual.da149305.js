import{k as w,l as S,o as d,p as k,t as T,d as L,m as P,s as I,a as N,q as A,r as q}from"./main.cb79c6a8.js";import E from"./component.chartComparison.2145fb22.js";import F from"./component.percentageBarMulti.531ea37c.js";var U={globals:{elem:document.querySelector(".perpetual")},init(){document.querySelector(".perpetual-list-content .js-refresh").addEventListener("click",()=>this.setPerpetuals()),document.querySelector("body.mobile.unknown")&&this.setPerpetuals();const e={childList:!0,attributes:!0,attributeFilter:["sidenav-activechange","sidenav-refreshprice"]};new MutationObserver(t=>{console.log("sidenav refreshed from Perpetual!");for(let n of t)if(n.type==="attributes")switch(n.attributeName){case"sidenav-activechange":document.querySelector("body.mobile.unknown")||this.setPerpetuals();break}this.globals.init=!1}).observe(this.globals.elem.querySelector(".sidenav"),e)},setPerpetuals(){console.log("setPerpetuals()");const e=document.querySelector(".perpetual-list"),a=document.querySelector(".perpetual-info");w(e),w(a);const t=e.querySelector(".comp-dynamic-table");t.textContent="",t.innerHTML=`
            ...
            </div>`,this.setPerpetualInfo(),S().then(n=>{const r=[],u=[];let l=0;n.forEach(async(i,m)=>{try{const o=await d(i,"name"),s=await d(i,"symbol"),c=await d(i,"aum"),v=await d(i,"supply"),f=await d(i,"balance"),b=await d(i,"leverage"),g=await d(i,"strike"),h=await d(i,"notional"),p=await d(i,"params"),x=v>0?c/v:1,y=`$${(v>0?c/v*f:0).toFixed(2)}`;u.push({Name:o,Symbol:s,Address:i,Description:`${p[2]} ${s}<br>Leverage achieved by buying ${(Number(p[3])/86400).toFixed(0)}-day options`,MarketCap:`$${c.toFixed(0)}`,UnitAsset:x,Holding:y,Leverage:b,SetLevel:p[0],CriticalLevel:p[1],Target:Math.ceil((p[0]+p[1])/2),Direction:p[2],Tenor:p[3],Strike:g,Notional:h}),r.push([o,p[2],b.toFixed(2)+"x",y,`$${x.toFixed(2)}`]),l++,l===n.length&&(w(e,!1),w(a,!1),this.setPerpetualInfo(u[0]),k.setDynamic(t,r),n.length>1&&t.querySelectorAll("tbody tr").forEach(($,C)=>{$.classList.add("cursor"),$.addEventListener("click",()=>{this.setPerpetualInfo(u[C])},!1)}))}catch(o){console.error(o),console.log("Fail! refresh data load...");const s=setTimeout(()=>{this.setPerpetuals(),clearTimeout(s)},5e3)}})}).catch(n=>{console.error(n)})},setPerpetualInfo(e){const a=document.querySelector(".perpetual-info-content");if(!e){a.textContent="";return}a.innerHTML=`
